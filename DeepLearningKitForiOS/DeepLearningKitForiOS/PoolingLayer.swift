...
func addPoolingCommandToCommandBufferCached(commandBuffer: MTLCommandBuffer,
    ...
    layer_string: String) -> MTLBuffer {
        ...
        let output = createFloatNumbersArray(count: outputCount)
        let (_, computePipelineState, _) = setupShaderInMetalPipeline(shaderName: poolingMethod, metalDefaultLibrary: metalDefaultLibrary, metalDevice: metalDevice)
        
        let outputMetalBuffer = createOrReuseFloatMetalBuffer(name: "outputMetalBuffer", data: output, cache: &layer_data_caches, layer_number: layer_number, metalDevice: metalDevice)
        //        let outputMetalBuffer = createFloatMetalBuffer(output, metalDevice: metalDevice)
        
        let sizeParamMetalBuffer = createOrReuseShaderParametersMetalBuffer(name: "sizeParamMetalBuffer", data: size_params, cache: &layer_data_caches, layer_number: layer_number, metalDevice: metalDevice)
        ...
        let poolingParamMetalBuffer = createOrReusePoolingParametersMetalBuffer(name: "poolingParamMetalBuffer", data: pooling_params, cache: &layer_data_caches, layer_number: layer_number, metalDevice: metalDevice)
